...
SELECT REGION,COUNT(SALES_AMOUNT) AS [TOTAL_SALES]
FROM SALES_DATA
GROUP BY REGION
...
SELECT REGION , SUM(SALES_AMOUNT) AS [MIN_SALES]
FROM SALES_DATA
WHERE YEAR = 2023
GROUP BY REGION
...
SELECT TOP 1 REGION , SUM(SALES_AMOUNT) AS [MIN_SALES]
FROM SALES_DATA
...
SELECT  TOP 1 PRODUCT ,SUM(SALES_AMOUNT) AS [HIGH_SALES]
FROM SALES_DATA
WHERE YEAR = 2023
GROUP BY PRODUCT
...
SELECT  REGION,SUM(SALES_AMOUNT) AS [HIGH_SALES]
FROM SALES_DATA
WHERE PRODUCT = 'TV'
GROUP BY REGION
...
SELECT YEAR, REGION ,COUNT(SALES_AMOUNT) AS[ORDER_COUNT]
FROM SALES_DATA
GROUP BY YEAR , REGION
ORDER BY YEAR , REGION

--2. Display Regions with Maximum Sales Amount Exceeding 1000 in Any Year, Sorted by Region 
SELECT REGION ,MAX(SALES_AMOUNT) AS[MAX_COUNT]
FROM SALES_DATA
GROUP BY YEAR , REGION
...
SELECT YEAR ,SUM(SALES_AMOUNT) AS[TOTAL_AMOUNT]
FROM SALES_DATA
GROUP BY YEAR 
...
SELECT   TOP 1YEAR,REGION ,SUM(SALES_AMOUNT) AS [TOTAL_SALES]
FROM SALES_DATA
